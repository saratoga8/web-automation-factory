...
export class PuppeteerElementBuilder implements ElementBuilder {
    ...
    async createContainingText(
        info: Elements.ElementInfo
    ): Promise<ContainsText> {
        const getElementTxtByValueAttr = () => (element: Element) =>
    ...
    async createTextEditable(
        info: Elements.ElementInfo
    ): Promise<TextEditable> {
        const page = await this.getPage()
